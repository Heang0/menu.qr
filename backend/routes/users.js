...
const Store = require('../models/Store'); // Import Store model as admins are linked to stores
    ...
    try {
        ...
        if (user) {
            ...
            res.status(201).json({
                ...
                role: user.role,
                storeId: store._id,
                publicMenuUrl: `/menu_display.html?storeId=${store._id}` // For frontend convenience
            });
        } else {
...
router.delete('/:id', protect, authorizeRoles('superadmin'), async (req, res) => {
    try {
        ...
        if (admin && admin.role === 'admin') {
            ...
            await User.deleteOne({ _id: admin._id }); // Use deleteOne for Mongoose 6+
